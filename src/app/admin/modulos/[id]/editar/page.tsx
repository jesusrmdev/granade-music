...
async function getModule(id: string) {
  try {
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/modules?id=eq.${id}&select=*&limit=1`,
...
export default async function EditModulePage(props: { params: Promise<{ id: string }> }) {
  await checkAdmin()
  ...
  return (
    <section className="mx-auto flex w-full max-w-6xl flex-1 flex-col px-4 py-8 sm:px-6">
      ...
      <ModuleForm action={updateModule} courseId={mod.course_id} defaultValues={mod} />
    </section>
  )
}
